    ...
    protected void apply(Map<Identifier, Research> prepared, ResourceManager manager, ProfilerFiller profiler) {
        ...
        prepared.forEach((id, entry) -> {
            if (researchCostMultiplier > 1) {
                ...
                entry = new Research(entry.titleText(), entry.descriptionText(), new ResearchCriterion<>(entry.trigger().criterion(), cost), entry.prerequisiteIds(), entry.recipeUnlocks(), entry.rewards(), entry.display(), entry.sizeSettings());
            }
